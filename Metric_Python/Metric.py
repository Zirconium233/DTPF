...
def preprocess_to_gray(img):
    """
    Preprocess the input image to grayscale.
    Supported formats:
    - Single channel: (H, W)
    - Three channels: (H, W, C) or (C, H, W)
    """
    ...
    if len(img.shape) == 3:
        # Determine the position of the channel dimension
        if img.shape[0] == 3:  # channels first (3, H, W)
            # Transpose to (H, W, C)
            img = np.transpose(img, (1, 2, 0))
        # Now the image format is unified to (H, W, C)
...
def preprocess_to_HWC(img):
    """
    Preprocess the input image to (H, W, C) format, automatically determining the original channel dimension position.
    Supported formats:
    - Three channels: (H, W, C) or (C, H, W)
    """
    if len(img.shape) == 3:
